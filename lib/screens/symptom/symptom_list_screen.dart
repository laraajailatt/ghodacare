...
import 'package:app_ghoda/models/symptom_model.dart';
import 'package:app_ghoda/screens/symptom/add_symptom_screen.dart';
import 'package:app_ghoda/screens/symptom/symptom_detail_screen.dart';
...
class _SymptomListScreenState extends State<SymptomListScreen> {
  ...
  Future<void> _loadSymptoms() async {
    ...
    try {
      ...
      setState(() {
        _symptoms = symptomsData.map((data) => SymptomModel.fromJson(data)).toList();
        _symptoms.sort((a, b) => b.date.compareTo(a.date)); // Sort by date, newest first
  ...
  void _applyFilter(String filter) {
    setState(() {
      ...
      if (filter == 'All') {
        ...
        _filteredSymptoms = _symptoms.where((symptom) => symptom.date.isAfter(lastWeek)).toList();
      } else if (filter == 'Last Month') {
        final lastMonth = DateTime.now().subtract(const Duration(days: 30));
        _filteredSymptoms = _symptoms.where((symptom) => symptom.date.isAfter(lastMonth)).toList();
      } else if (filter == 'Mild' || filter == 'Moderate' || filter == 'Severe') {
        _filteredSymptoms = _symptoms.where((symptom) => symptom.severity == filter).toList();
      }
    });
  }
