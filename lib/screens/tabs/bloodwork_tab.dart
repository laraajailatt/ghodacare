...
class _BloodworkTabState extends State<BloodworkTab> {
  ...
  Future<void> _loadBloodworkData() async {
    ...
    try {
      ...
      final bloodworkModels = bloodworks
          .map((data) => BloodworkModel.fromJson(data))
          .toList();
  ...
  bool _isImproving(double latest, double previous, double min, double max) {
    ...
    if (!_isWithinRange(latest, min, max) && !_isWithinRange(previous, min, max)) {
      // Calculate distance to nearest boundary
      final latestDistance = min > latest 
          ? min - latest 
          : latest - max;
      final previousDistance = min > previous 
          ? min - previous 
          : previous - max;
        ...
        Expanded(
          child: _buildSummaryCard(
            ...
            _recentBloodworks.isEmpty 
                ? 'No tests yet' 
                : 'Last test date',
            _recentBloodworks.isEmpty ? Icons.science_outlined : Icons.calendar_today,
      ...
      children: [
        Expanded(
          child: ElevatedButton.icon(
            onPressed: () {
              Navigator.pushNamed(context, '/add_bloodwork')
                  .then((value) {
                if (value == true) {
                  _loadBloodworkData();
                }
